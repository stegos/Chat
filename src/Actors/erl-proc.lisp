...
(defmethod dispatch-message ((*current-actor* process) &rest msg)
  (flet ((handle-below ()
           (handler-case
               (call-next-method)
             (error (c)
    ...
    (dcase msg
      ...
      (:unlink (proc)
       (setf (process-links *current-actor*)
             (delete proc (process-links *current-actor*))))
      
      (:exit (from reason)
       (flet ((kill-and-propagate ()
                (setf (car (actor-busy *current-actor*)) :exit)
                (let ((links (shiftf (process-links *current-actor*) nil)))
                  (dolist (proc links)
                    (send proc :unlink *current-actor*)
...
(defun spawn-link (fn &rest args)
  ...
  (let ((proc (make-process (lambda (&rest args)
                              ...
                              (exit :normal))
                            :link t)))
    (when (typep (current-actor) 'process)
      (push proc (process-links (current-actor))))
